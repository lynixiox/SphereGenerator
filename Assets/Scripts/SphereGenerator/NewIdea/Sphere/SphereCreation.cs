...
public class SphereCreation : MonoBehaviour
{
    ...
    public TerraManager terraManager;
    public TerrainManager terrainManager;
    /*
     * Spheres are easier to create from triangular points
     * so what i need
     * Edges
     * Corners
     * Tiles
    ...
    void Start()
    {
        ...
        terraManager = TerraManager.instance;
        terrainManager = TerrainManager.Instance;
        terrainManager.tCreation.Setup(finalGrid);
        CreateObjects();
        //CreateMesh();
    }
    ...
    void CreateObjects()
    {
        for(int i = 0; i < finalGrid.tiles.Count;i++)
        {
            ...
            go.GetComponent<SoloTile>().SetupTile(finalGrid.tiles[i].position, finalGrid.tiles[i].corners, finalGrid.tiles[i]);
            TerraManager.instance.AddTile(go);
        }
    }
    void CreateMesh()
    {
    ...
    sHexGrid CreateZeroSizeSphere()
    {
        ...
        foreach (sCorner c in grid.corners)
        {
            for(int i = 0; i < 3;i++)
            {
                c.corners[i] = c.tiles[i].corners[(sTileMaths.CalcPos(c.tiles[i], c) + 1) % 5];
            }
        }
    ...
    sHexGrid CreateSubdividedSphere(sHexGrid previous)
    {
        ...
        foreach(sCorner c in nGrid.corners)
        {
            for(int k = 0; k < 3;k++)
            {
                c.corners[k] = c.tiles[k].corners[(sTileMaths.CalcPos(c.tiles[k], c) + 1) % (c.tiles[k].edgeCount)];
            }
        }
    ...
    void AddCorner(int id,sHexGrid grid, int t1, int t2, int t3)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            t[i].corners[sTileMaths.CalcPos(t[i], t[(i + 2) % 3])] = c;
            c.tiles[i] = t[i];
        }
    ...
    void AddEdge(int id, sHexGrid grid, int t1, int t2)
    {
        ...
        sCorner[] c =
        {
            grid.corners[t[0].corners[sTileMaths.CalcPos(t[0], t[1])].id],
            grid.corners[t[0].corners[(sTileMaths.CalcPos(t[0], t[1])+1)%t[0].edgeCount].id]
        };

        for(int i = 0; i < 2;i++)
        {
            t[i].edges[sTileMaths.CalcPos(t[i], t[(i + 1) % 2])] = e;
            e.tiles[i] = t[i];

            c[i].edges[sTileMaths.CalcPos(c[i], c[(i + 1) % 2])] = e;
            e.corners[i] = c[i];
        }
